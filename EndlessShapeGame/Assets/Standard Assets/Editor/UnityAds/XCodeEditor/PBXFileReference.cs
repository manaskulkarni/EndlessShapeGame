...
namespace UnityEngine.Advertisements.XCodeEditor
{
	public class PBXFileReference : PBXObject
	{
		...
		private void GuessFileType()
		{
			...
			string extension = System.IO.Path.GetExtension((string)_data[NAME_KEY]).Trim('"');
			string typeName = "file";
			string typePhase = "PBXResourcesBuildPhase";

			if(PBXFileReference.typeNames.ContainsKey(extension)) {
				typeName = PBXFileReference.typeNames[extension];
				typePhase = PBXFileReference.typePhases[extension];
			}

			this.Add(LASTKNOWN_FILE_TYPE_KEY, typeName);
			this.buildPhase = typePhase;
		}
