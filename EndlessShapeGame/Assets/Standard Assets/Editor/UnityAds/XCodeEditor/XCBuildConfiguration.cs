...
namespace UnityEngine.Advertisements.XCodeEditor
{
	public class XCBuildConfiguration : PBXObject
	{
		...
		protected const string OTHER_LINKER_FLAGS = "OTHER_LDFLAGS";
		...
		public bool AddOtherLinkerFlags(PBXList flags)
		{
			...
			foreach(string flag in flags) {
				...
				if(!((PBXDictionary)_data[BUILDSETTINGS_KEY]).ContainsKey(OTHER_LINKER_FLAGS)) {
					((PBXDictionary)_data[BUILDSETTINGS_KEY]).Add(OTHER_LINKER_FLAGS, new PBXList());
				} else if(((PBXDictionary)_data[BUILDSETTINGS_KEY])[OTHER_LINKER_FLAGS] is string) {
					string tempString = (string)((PBXDictionary)_data[BUILDSETTINGS_KEY])[OTHER_LINKER_FLAGS];
					((PBXDictionary)_data[BUILDSETTINGS_KEY])[OTHER_LINKER_FLAGS] = new PBXList();
					((PBXList)((PBXDictionary)_data[BUILDSETTINGS_KEY])[OTHER_LINKER_FLAGS]).Add(tempString);
				}
				
				if(!((PBXList)((PBXDictionary)_data[BUILDSETTINGS_KEY])[OTHER_LINKER_FLAGS]).Contains(flag)) {
					((PBXList)((PBXDictionary)_data[BUILDSETTINGS_KEY])[OTHER_LINKER_FLAGS]).Add(flag);
					modified = true;
				}
			}
